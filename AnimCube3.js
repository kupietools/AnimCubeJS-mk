...
function AnimCube3(params) {
  ...
  function init() {
    ...
    colors[5]  = rgbToHex(230, 230,  125);  // 5 - yellow grey
    colors[6]  = rgbToHex(187, 119,  68);  // 6 - orange grey
    colors[7]  = rgbToHex(230,125,125);  // 7 - red grey
    colors[8]  = rgbToHex(68,  119,  68);  // 8 - green grey
    colors[9]  = rgbToHex(125,    160, 230);  // 9 - blue grey
    colors[10] = rgbToHex(246, 246, 246);  // W - white
    colors[11] = rgbToHex(255, 255,   0);  // Y - yellow
    colors[12] = rgbToHex(245,  138,  0);  // O - orange
    colors[13] = rgbToHex(208,   0,   0);  // R - red
    colors[14] = rgbToHex(0,   192,   0);  // G - green
    colors[15] = rgbToHex(0,   160, 255);  // B - blue
    colors[16] = rgbToHex(176, 176, 176);  // L - light gray
    colors[17] = rgbToHex( 45,  45,  45);  // D - dark gray
  ...
  */
  ...
  window.florian={g,x,y,color,p,q,h};
    g.beginPath();
    for (var i=0; i < 4; i++) {
        ...
        if (spin) {
          if (moveAnimated) {
            if (currentAngle * phis < phit) {
              ...
              // NOTE FROM MK - don't know if this actyually works but not hurting anything
              while (Date.now() - time0 < 1)
                ;  // empty loop
                // END part I don't know actually works
              paint();
  ...
  function button() {
    ...
    if (buttonPressed == 3) {
    ...
    else if (buttonPressed == 4) { // play
      if (movePos == move[curMove].length) 
        clear();
      startAnimation(buttonAction[buttonPressed]);
      //this else clause added to reset to original scramble if play pressed after solved
    }
      ...
      setTimeout(startAnimation, 0, buttonAction[6]);
         //this setTimeout clause added to reset to original scramble if play pressed after solved - don't know if this is actually necessary but I think so.
    }
    else
      startAnimation(buttonAction[buttonPressed]);
    drawButtons = true;
